...
public class TicTacToeGame : Game
{
    ...
    {
        if (gameState != GameState.Ongoing || !IsValidMove(row, col))
        {
            return false;
        }
    ...
    public override bool CheckWinCondition(int row, int col)
    {
        ...
        // Check diagonal
        if (row == col && _board[0, 0] == currentPlayer && _board[1, 1] == currentPlayer && _board[2, 2] == currentPlayer)
            return true;


        // Check anti-diagonal
        if (row + col == 2 && _board[0, 2] == currentPlayer && _board[1, 1] == currentPlayer && _board[2, 0] == currentPlayer)
            return true;
    ...
    public override bool CheckDrawCondition()
    {
        ...
        return _movesMade >= 9; // replace this with your code
    }
